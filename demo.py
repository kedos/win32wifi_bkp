...
class WLAN_DISCONNECT(Structure):
    _fields_ = [
        ("Index", DWORD),
    ]
...
def scan(handle, iface):
    # Just calling this, will set the interface in scan mode.
    ret = WlanScan(handle,
...
if __name__ == '__main__':
    ...
    try:
        ...
        for iface in ifaces:
            ...
            # Just calling this, will set the interface in scan mode.
            scan(ClientHandle, iface)
            # ret = WlanScan(ClientHandle,
            #                byref(iface.InterfaceGuid),
            #                None,
            #                None,
            #                None
            ...
            # time.sleep(1)
            # print(ret)
            # ret = WlanDisconnect(ClientHandle,
            #                      byref(iface.InterfaceGuid),
            #                      None)
            if ret != ERROR_SUCCESS:
                exit(FormatError(ret))
